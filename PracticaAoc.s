...
Eano:				.asciiz "\nLa fecha introducida no se corresponde a un input valido para el calendario gregoriano\n"
...
__main:
				...
				bltz $v0, ErrAlg		#Error Algoritmo
				bgt  $v0, 7, ErrAlg		#Error Algoritmo
				...
				addi $s3, $s3, -1		#Restamos 1 a el numero del mes
				mul $s3, $s3, 20		#Calculamos el desplazamiento
				add $a0, $a0, $s3		#Sumamos el desplazamiento a la direccion
...
ErrAlg:
				la $a0, Ealg			#Se ha producido un error en el algoritmo
...
ErrM:									#Se ha producido un error con el mes introducido
...
Leer:					
				li $t1, 0				#Dejamos los temporales a 0
				li $t2, 0
				li $t3, 0
Dia:
...
Comprobar:			
				add $s2, $zero, $t1		#Guardamos dia, mes y anyo en s2 s3 y s4 respectivamente
				add $s3, $zero, $t2
				add $s4, $zero, $t3
				...
				beq $t2, 10, Octubre 	#Octubre
				beq $t2, 11, Mes30 		#Noviembre
				beq $t2, 12, Mes31 		#Diciembre

				
Octubre:				
				beq $t3, 1582, CJuliano  #Si el año es 1582 vamos a Cjuliano
				j Mes31					 #Si no comprobamos de forma normal

CJuliano:									#El primer dia del calendario gregoriano es 15/10/1582
				addi $t5, $zero, 14			#Si el dia es >14 comprobamos de forma normal
				bgt $t1, $t5, Mes31
				j Erroranyo					#Si no vamos a Erroranyo
...
Bisiesto:
				...
				addi $t5, $zero, 400	
				div $t3, $t5			# Año/400
				mfhi $t5 				#guardamos el resto en t35
